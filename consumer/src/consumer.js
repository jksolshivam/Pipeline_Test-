...
const configManager = {
  async fetchConfig() {
    try {
      ...
      const oldTopics = allRoutes.map((r) => r.topic).sort();
      const newTopics = newRoutes.map((r) => r.topic).sort();
 
      const topicsChanged =
        JSON.stringify(oldTopics) !== JSON.stringify(newTopics);
 
      allRoutes = newRoutes;
 
      if (topicsChanged && consumer) {
        console.log("🔄 Topics changed, restarting consumer...");
...
async function restartConsumer() {
  try {
    console.log("🛑 Restarting consumer safely...");
